...
def search_furniture(keyword):
	...
	try:
		...
		search_label_res = conn.execute("SELECT * FROM Furniture WHERE labels=?",
			                             [keyword]).fetchall()
		conn.commit()
		...
		for f in search_label_res:
			...
			furniture_list.append({"fid": fid, "owner": owner, "title": title, "labels": labels, 
				                    "status": status, "image_url": image_url, "description": description})
